...
class GP370ControllerTab(DeviceTab):
    def initialise_GUI(self):
        ...
        visa_resource =  self.settings['connection_table'].find_by_name(self.settings["visa_resource"]).BLACS_connection
        self.ui.visa_resource_label.setText(visa_resource) 
    ...
    def check_connection_and_read_pressure(self, *args):
        ...
        self.ui.current_pressure(value)
        
        status_text = 'Working!'
        icon = QIcon(':/qtutils/fugue/tick')
        pixmap = icon.pixmap(QSize(16, 16))
        self.ui.status_icon.setPixmap(pixmap)
        elf.ui.connection_status.setText(status_text)
        
    def initialise_workers(self):
        # Create and set the primary worker
        self.create_worker("main_worker",GP370Worker,{'com_port':self.visa_resource})
    ...
    def transition_to_buffered(self,device_name,h5file,initial_values,fresh):
        with h5py.File(self.h5_file,'a') as hdf5_file:
            try:
                ...
                gp370data = hdf5_file.create_group('/data/GP370')
